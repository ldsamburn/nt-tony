...
//returns a comma delimited CString of all the Values
//all the guys on the left of = sign  ... jj
CString CIniFile::GetValues(const CString& keyname)
{
	CString strVals=_T("");

	int keynum = FindKey(keyname);
	int numvalues = GetNumValues(keyname);

	for(int i=0;i < numvalues; i++)
		if(strVals.IsEmpty())
			strVals=keys[keynum].values[i];
		else
			strVals+=_T(",") + keys[keynum].values[i];
