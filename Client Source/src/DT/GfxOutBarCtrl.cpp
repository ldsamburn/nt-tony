...
#define _ID_GFX_PROPERTIES	_T("Properties")
...
BEGIN_MESSAGE_MAP(CGfxOutBarCtrl, CWnd)
	...
	ON_COMMAND(ID_GFX_PROPERTIES, OnGfxProperties)
	ON_UPDATE_COMMAND_UI(ID_GFX_PROPERTIES, OnUpdateGfxProperties)
	//}}AFX_MSG_MAP
...
//	if(index>=0)
		iSelFolder = index; //jj originallly was just this line, but iSelFolder was going to -1
///	else
//		iSelFolder =0;
...
void CGfxOutBarCtrl::RemoveFolder(const int index)
{
	...
	if (iSelFolder < 0 && GetFolderCount() > 0) iSelFolder = 0; //jj was iSelFolder = 0
	Invalidate();
}
...
void CGfxOutBarCtrl::OnSize(UINT nType, int cx, int cy) 
{
	...
	/* ADD THIS TO PROPERLY SET bUpArrow/bDownArrow... JGJ */
	/*if (max > 0) {
		int f, l;
		f = l = 0;
		TRACE0("Calling GetVisibleRange\n");
		GetVisibleRange(iSelFolder, f,l);
		TRACE0("Called GetVisibleRange\n");

		bUpArrow = (f > 0);
		bDownArrow = (l < GetItemCount() - 1);

		bUpArrow = true; //jgj debug
...
void CGfxOutBarCtrl::OnRButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (iHitInternal1 == htItem)
	{
		...
		//jj added properties 
		cMenu.AppendMenu(MF_SEPARATOR); 
		cMenu.AppendMenu(MF_STRING, ID_GFX_PROPERTIES, _ID_GFX_PROPERTIES);
		//jj added properties
	}
...
//////////////////////////////////////////////////////////
void CGfxOutBarCtrl::OnGfxProperties() 
{

	ProgramStruct* pProgramStruct=(ProgramStruct*)GetItemData(iHitInternal2);
	if(!pProgramStruct->m_nFlags) //client program
	{
		SHELLEXECUTEINFO sei;
		TCHAR lpszPATH[_MAX_PATH];
		_stprintf(lpszPATH,_T("%s%s"),pProgramStruct->m_strWorkingDir,
	...
	{
		CPropDlg dlg(NULL,pProgramStruct->m_strProgramTitle,pProgramStruct->m_strProgramFile,
		pProgramStruct->m_strCmdLineArgs,pProgramStruct->m_strWorkingDir);
		dlg.DoModal();
	}
}

void CGfxOutBarCtrl::OnUpdateGfxProperties(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(TRUE);
}
...
CWnd * CGfxOutBarCtrl::GetFolderChild(int iFolder)
{
	if (GetFolderCount())
	{
		if(iSelFolder<0) iSelFolder=0; //jj added this line to fix blitzed iSelFolder
