...
# The set_perms.sh can then be run to re-constitute the permissions.
    ...
    then
        ...
        echo "                       'listing_file' file in the directory"
        echo "                       defined by arg-1"
        echo " ";echo " "
        exit 0
    fi

    # The echo command on HP suppresses multiple spaces to 1 space
    os="OTHER" # SunOS, Linux
    os=`uname -a | grep ^HP-UX`
    if [ $? = "0" ]
    then
        os="HP"
    fi
    ...
    then
        ...
        tar tvf /tmp/perms.tar > $2/listing_file
        rm /tmp/perms.tar
        echo " ";echo " ";echo " "
    else
    ...
    then
        ...
        then
            ...
            tar tvf /tmp/perms.tar > $model_dir/listing_file 
        else
    ...
    do
        ...
        #Extract relative pathname from listing_file
...
#        esac
        ...
        if [ "$fullpath" = "/h/NTCSSS/post-install.log" ]
        then
            continue
        fi
        ...
        if [ -h "$fullpath" ]
        then
            echo "Skipping link file ($p)"
            continue
        fi

        if [ "$2" = "default" -o "$3" = "default" ]
        then
            envfile=`echo $fullpath | grep "/h/NTCSSS/env"`
            if [ "$?" = "0" ]
            then
                echo "Skipping NTCSSS/env files"
                continue
            fi
        fi
        ...
        then
            ...
            owner=`echo $pathname | cut -c 10-20 | cut -d "/" -f 1`
            group=`echo $pathname | cut -c 10-20 | cut -d "/" -f 2`
        else
            # For SunOS, Linux
            perms=`echo $pathname | cut -c 2-10`
            a1=`echo "$perms" | cut -c 2`; a2=`echo "$perms" | cut -c 3`
            a3=`echo "$perms" | cut -c 4`; a4=`echo "$perms" | cut -c 5`
            a5=`echo "$perms" | cut -c 6`; a6=`echo "$perms" | cut -c 7`
            a7=`echo "$perms" | cut -c 8`; a8=`echo "$perms" | cut -c 9`
            a9=`echo "$perms" | cut -c 10`
            owner=`echo $pathname | cut -c 11-21 | cut -d "/" -f 1`
            group=`echo $pathname | cut -c 11-21 | cut -d "/" -f 2`
        fi
        ...
        echo "CHMOD CMD: chmod $accum"
        echo "CHOWN Cmd: chown $owner"
        echo "CHGRP Cmd: chgrp $group"
        echo "... $p"
        ...
        echo "    chown $owner $fullpath" >> $model_dir/set_perms.sh
        echo "    chgrp $group $fullpath" >> $model_dir/set_perms.sh
    ...
    } < $model_dir/listing_file

#The env files can change from install to install...
    if [ "$2" = "default" -o "$3" = "default" ]
    then
        if [ $model_dir = "/h/NTCSSS" ]
        then
            echo "if [ ! -d /h/NTCSSS/env ]" >> $model_dir/set_perms.sh
            echo "then" >> $model_dir/set_perms.sh
            echo "    echo \"/h/NTCSSS/env NO LONGER EXISTS !!\" >> \$spl" >> $model_dir/set_perms.sh
            echo "    missing_files=\"y\"" >> $model_dir/set_perms.sh
            echo "else" >> $model_dir/set_perms.sh
            echo "    chmod -R 755 /h/NTCSSS/env" >> $model_dir/set_perms.sh
            echo "fi" >> $model_dir/set_perms.sh 
        fi
    fi
