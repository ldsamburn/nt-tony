...
/**********************
 PUBLIC FUNCTIONS
**********************/
void        NtcssError_set_error_code(int);
const char *NtcssError_get_error_message(void);
void        NtcssError_reset(void);
int         NtcssError_occurred(void);

/**********************
PUBLIC DATA
**********************/
NtcssError_class NtcssError = {0,"",
				 NtcssError_set_error_code,
				 NtcssError_get_error_message,
				 NtcssError_reset,
				 NtcssError_occurred};
...
int  NtcssError_occurred()

{
  return(NtcssError.error_code != NO_ERROR);
}

/*****************************************************************************/

void NtcssError_reset()

{
  NtcssError.error_code = NO_ERROR;
}


/*****************************************************************************/

void NtcssError_set_error_code( x )

     int  x;
{
  NtcssError.error_code = x;
}


/*****************************************************************************/

const char  *NtcssError_get_error_message()

{
  switch(NtcssError.error_code)
    {
    case NO_ERROR:
    ...
    case ERROR_SERVER_ERROR:
      return(NtcssError.server_error_message + NUMBER_HEADER_SIZE);
