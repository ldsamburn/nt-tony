 ...
 *
 * Testing portions written by Denton Byers (dbyers@ch.inri.com)
 * Inter-National Research Institute, 1997
 */
...
/*
static int NTCSSencrypt( char *, char * );
static int NTCSSdecrypt( char *, char * );
*/
...
/***********************************************/
/* VECTORS DEFINED ONLY FOR TESTING PURPOSES   */
/*      (Refer to bottom of this file)         */
/* Testing vectors written by Denton Byers,    */
/* for Inter-National Research Institute, 1997 */

/***** single-key single-plaintext test vector
unsigned char Key[8] = {0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef};
unsigned char Plain[8]={0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xe7};
*****/

/****  double-key single-plaintext test vector
unsigned char Key[16] = {0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef,0xfe,0xdc,0xba,0x98,0x76,0x54,0x32,0x10};
unsigned char Plain[8]={0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xe7};
****/

/****  double-key double-plaintext test vector
unsigned char Key[16] = {0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef,0xfe,0xdc,0xba,0x98,0x76,0x54,0x32,0x10};
unsigned char Plain[16]={0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef,0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xff};
****/

/**** Triple-length key, single-length plaintext test vector
...
/***** triple-key double-plaintext test vector
unsigned char Key[24] = {0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef,0xfe,0xdc,0xba,0x98,0x76,0x54,0x32,0x10,0x89,0xab,0xcd,0xef,0x01,0x23,0x45,0x67};
unsigned char Plain[16] ={0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef,0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xff};
*/

/**********************************************************************/
/* FUNCTIONS USED ONLY FOR TESTING PURPOSES                           */
/* Testing functions written by Denton Byers,                         */
/* for Inter-National Research Institute, 1997                        */
...

{
  ...
  printf("expected cipher value followed by actual result: \n");

 **** for single-key double-plaintext
  deskey(&Key, EN0);
  des(&Plain,&Cipher);
  printf(" c9 57 44 25 6a 5e d3 1d\n");
  for (a=0;a<16;a++)
    printf(" %2x", Cipher[a]);
  printf("\n");
*****/

/***** double-key, single-plaintext
  des2key(&Key, EN0);
  Ddes(&Plain,&Cipher);
  printf(" 7f 1d 0a 77 82 6b 8a ff\n");
  for (a=0;a<8;a++)
    printf(" %2x", Cipher[a]);
  printf("\n");
*****/
...
/**** Double-length key, double-length plaintext
  des2key(&Key, EN0);
  D2des(&Plain,&Cipher);
  printf(" 27 a0 84 40 40 6a df 60 27 8f 47 cf 42 d6 15 d7\n");
  for (a=0;a<16;a++)
    printf(" %2x", Cipher[a]);
  printf("\n");
****/
...
/*****************************************************************************/
void  makekey( register char *aptr,  /* NULL-terminated  */
               register unsigned char *kptr)     /* unsigned char[8] */

{
  register unsigned char *store, temp;
      ...
      for( i = 0; i < 8 && (*aptr != '\0'); i++ )
        {
          temp = *aptr;
          *store++ ^= *aptr & 0x7f;
          /*modified to keep *aptr string from getting clobbered*/
          /* *aptr++ = '\0'; */
          aptr++;
        }
      Ddes(kptr, kptr);
...
static void D3des(from, into)   /* amateur theatrics */
     ...
     /* Note this function is not for data encryption but rather
    key hashing */
...
void make3key(aptr, kptr)
     ...
     register const char *aptr;         /* NULL-terminated   */
