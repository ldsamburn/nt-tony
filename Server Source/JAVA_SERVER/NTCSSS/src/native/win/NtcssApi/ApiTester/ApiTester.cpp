...
// ApiTester.cpp : Defines the entry point for the console application.
//
...
static void  NtcssGetSystemInfo_test(void);
static void  NtcssGetSystemInfo_get_parameters(void);
static void  NtcssGetSystemInfo_print_parameters(void);
static void  NtcssGetSystemInfo_print_results(void);
...
void getBaseNtcssDir(char* path, const int size)
{
   HMODULE hModule;
...
void  read_string(char  *buffer,int   silent)
{
  char  *start;
...
void  get_number(const char  *label, int  *number)
{
...
void  get_number(const char  *label, bool  *number)
{
  char local_string[100];  
  ...
  if (local_string[0] == '*')
    ...
    if (local_string[0] != '=') 
      *number = atoi(local_string) ? true : false;    
}


/*****************************************************************************/

void  get_string(const char  *label, char *string_data,int silent)
{
  char   local_string[100];
...

{
  ...
  getBaseNtcssDir(exec_file_name, MAX_PATH_LEN);
  strcat(exec_file_name, "\\bin\\writenow");
...
//                 process_description.prog_args_str,NOT_SILENT);
                 process_description.sdi_tci_file_str,NOT_SILENT);
      if (let_the_rest_be_default)
        return;
    }
}
...
//             "determine this  host's name!");
  ...
  getBaseNtcssDir(print_filename, NTCSS_SIZE_FILENAME);
  strcat(print_filename, "\\logs\\command_server.pid");
...
static void  NtcssGetSystemInfo_test()

{
  clear_string(host_info.osName);
  clear_string(host_info.osArch);
  clear_string(host_info.osVersion);
  clear_string(host_info.fileSeparator);
  clear_string(host_info.pathSeparator);
  clear_string(host_info.lineSeparator);  
  success = NtcssGetSystemInfo(host_name,&host_info);
}
/*****************************************************************************/

static void  NtcssGetSystemInfo_get_parameters()

{
 get_string("HostName: ", host_name,NOT_SILENT); 
}

/*****************************************************************************/

static void  NtcssGetSystemInfo_print_results() 

{
  printf("%s's OS name:        %s\n", host_name, host_info.osName);
  printf("%s's OS arch:        %s\n", host_name, host_info.osArch);
  printf("%s's OS version:     %s\n", host_name, host_info.osVersion);
  printf("%s's file separator: %s\n", host_name, host_info.fileSeparator);
  printf("%s's path separator: %s\n", host_name, host_info.pathSeparator);
  printf("%s's line separator: %s\n", host_name, host_info.lineSeparator);
}

/*****************************************************************************/
static void  NtcssGetSystemInfo_print_parameters() 
...
void  load_test(const char  *name, void  (*test)(void), void  (*get_params)(void), 
                void  (*prt_params)(void), void  (*prt_results)(void))
{
...
************************************************************/
  ...
  load_test("NtcssGetSystemInfo",
            NtcssGetSystemInfo_test,
            NtcssGetSystemInfo_get_parameters,
            NtcssGetSystemInfo_print_parameters,
            NtcssGetSystemInfo_print_results);
}
