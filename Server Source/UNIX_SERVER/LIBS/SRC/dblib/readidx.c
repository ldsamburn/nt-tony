...
off_t offset;
{
	...
	struct iovec	iov[2];
		...
		/* Position index file and record the offset.  dbNextrec()
	...
	if ( (/*i = */(size_t)read(db->idxfd, db->idxbuf, db->idxlen)) != db->idxlen)
