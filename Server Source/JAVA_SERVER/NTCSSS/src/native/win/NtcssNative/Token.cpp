...
CStdString CToken::GetNextToken()
{
	...
	int pos, len;
	CStdString newtok = m_strLeft; // Our own working copy of the string left
	len = newtok.GetLength();
	pos = newtok.Find(m_strToken,0);
	if (pos == -1)
	{
		// This should never happen
		//TraceEx("Opps. I did not think this would happen.\nPlease Contact Richard Case: case@dcs.kcl.ac.uk");
	}
	m_bFoundAny = TRUE;
