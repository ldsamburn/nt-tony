...
//Description: A character output stream that sends output to a printer.
...
public class HardcopyWriter extends Writer
{
   ...
   protected int x0, y0;                    // Uper-left corner insid maregin
   protected int width, height;             // Size (in dots) inside margin
   ...
   {
      // Get the PrintJob object with which we'll do all the printing. The
      //  call is synchronized on the static printprops object, which means
      //  that only one print dialog can be popped up at a time. If the user
      //  clicks Cancel in the print dialog, throw an exception.
      ...
      if (System.getProperty("os.name").regionMatches(true, 0, "windows", 0, 7)) {
         // User screen dpi, which is what the PrintJob tries to emulate, anyway
      ...
      // Compute coordinates of the upper-left corner of the page. I.e. the
      //  coordinates of (leftmargin, topmargin). Also compute the width and
      //  height inside of the margins.
      ...
      // Compute the data/time string to display in the page header
   ...
   public void write(char[] buffer, int index, int len)
   {
      synchronized(this.lock) {
         ...
         for (int i = index; i < index + len; i++) {
            ...
            // If its some other non-printing character, ignore it.
            ...
            if (charnum >= chars_per_line) {
               newline();
               if (page == null)
                  newpage();  // and start a new page, if necessary
            }
            ...
            //  printer users we need to position each character individually.
   ...
   // Set the font style. The argument should eb one of the font stype constants
   ...
   protected void newline()
   {
      ...
      if (linenum >= lines_per_page) {  // If we've reached the end of the page
         page.dispose();                //  send page to the printer but
         page = null;                   //  don't start a new page yet.
      }
   }
   ...
   protected void newpage()
   {
      ...
      // Set the basic monspaced font for the rest of the page
      page.setFont(font);
   }
